...
public class Test {
    ...
    public static void main(String[] args) {
        ...
        Animal dog = new Dog();
        animal.run();
        dog.run();    // 对重写的方法, 看new后面的类型是什么
        animal.eat();
        dog.eat();    // 对于静态方法是看变量的类型
        
        dog.break();   //对没有重写的方法, 看new前面的类型是什么  //dog不能调用这个方法的, 因为编译器只知道dog的类型为Animal
...
+ **对于实例方法（非静态方法），如果调用方法是重写了的, 则调用的是引用所指向的对象的实际类型中(new 后面那个)的方法,。**
+ **对于静态方法，调用的是引用类型(具体生成的对象(new 前面那个))所对应的类中的方法，与实际对象的类型无关。**
...
<img src="%E7%AC%94%E8%AE%B0.assets/image-20240804170424467.png" alt="image-20240804170424467" style="zoom: 33%;" />
...
+ 方法2论字符串常量池中是否已经存在相同内容的字符串对象，这种方式都会创建一个新的字符串对象，并将其存储在堆内存中。![image-20240514131805982](%E7%AC%94%E8%AE%B0.assets/image-20240514131805982.png)
...
注释：上述代码中，我们使用了compareTo方法来比较两个Integer变量的大小。如果返回值小于0，则表示num1小于num2；如果返回值大于0，则表示num1大于num2；如果返回值等于0，则表示num1等于num2。
...
只能传入 T 类型的数据,  方法实例化的数据类型，就是 T 的数据类型。( 根据上面的代码理解也就是，如果Generic02<Double>，那么就限制 T 为 Double 类型。)
...
class Generic03 {
    public <T> void toGeneric03(T[] arr) {   // 参数是T 返回值必要使用<T>
...
// -------------
// 自定义类, 里面使用了parseObject
public static <T> T verify(String token, Class<T> clazz){
    DecodedJWT decodedJWT = JWT.require(Algorithm.HMAC256(SECRET)).build().verify(token);
    // 将JWT主体转换为指定类型的对象并返回
    return JSON.parseObject(decodedJWT.getSubject(), clazz);
}
//调用
UserVo user = new UserVo();
jwtUtil.verify(token,UserVo.class);
...
public static <T> String Token(T data, Long expireDate){
    ...
    else if (data instanceof SomeOtherType) 
    {  // 用于检查 data 对象的类型是否是 SomeOtherType 或其子类
        SomeOtherType someOtherType = (SomeOtherType) data;
        // 提取 SomeOtherType 对象的用户名和用户ID
        // userName = someOtherType.getName();
        // userId = someOtherType.getId();
    }
...
![image-20230923101240454](笔记.assets/image-20230923101240454.png)





[解决 Web server failed to start. Port xxx was already in use.端口占用问题_web server failed to start. port 8999 was already _heiqi_whf的博客-CSDN博客](https://blog.csdn.net/weixin_42188778/article/details/126995184)
...
Lambda基本语法: (parameters) -> expression 或 (parameters) ->{ statements; }表达式的基本使用
...
Lambda表达式本质是一个匿名函数，只需要关心：参数列表 方法体
...
运行结果：
![请添加图片描述](%E7%AC%94%E8%AE%B0.assets/22c41c48c6f140bcaf20fcb335e63d7e.png)
...
运行结果：
<img src="%E7%AC%94%E8%AE%B0.assets/image-20231116164224659.png" alt="image-20231116164224659" style="zoom:67%;" />
...
![img](%E7%AC%94%E8%AE%B0.assets/a09eba1fc9e971d19f427a7553f18af3.png
