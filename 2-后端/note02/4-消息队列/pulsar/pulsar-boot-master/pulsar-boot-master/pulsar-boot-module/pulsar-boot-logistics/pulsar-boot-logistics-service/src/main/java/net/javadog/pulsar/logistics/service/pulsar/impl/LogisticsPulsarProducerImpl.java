package net.javadog.pulsar.logistics.service.pulsar.impl;
...
@Slf4j
@Component
public class LogisticsPulsarProducerImpl implements LogisticsPulsarProducer {

    @Value("${pulsar.url}")
    private String url;
    @Value("${pulsar.logistics-topic}")
    ...
    @Override
    public void handler(String orderId, long delay) {
        log.debug("物流成功消息至Pulsar，orderId：{}，delay：{}", orderId, delay);
        //异步发送
        CompletableFuture<MessageId> future = producer.newMessage().deliverAfter(delay, TimeUnit.SECONDS).value(orderId.getBytes()).sendAsync();
        future.handle((v, ex) -> {
            if (ex == null) {
                log.debug("Message persisted2: {}", orderId);
            } else {
                log.debug("发送Pulsar消息失败msg:【{}】 ", orderId, ex);
            }
            return null;
        });
        log.debug("Message persisted: {}", orderId);
    }
}
