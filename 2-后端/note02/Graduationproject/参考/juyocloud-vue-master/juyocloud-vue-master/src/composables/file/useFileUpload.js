...
} from '@/composables/file/useTableOperator'
...
// 正在上传或已完成文件映射表 key: fileInfo.home value: fileInfo
...
export default function useFileUpload() {
			...
			getFilesByDataTransferItemList(items).then((fileList) => {
				...
				if (hiddenFileIndexArray.length > 0) {
					ElMessageBox.confirm(
						`检测到有 ${hiddenFileIndexArray.length} 个 .DS_Store 文件，是否上传时跳过这些文件`,
						'提示',
						{
					...
					if (fileList.length > 100) {
						ElMessageBox.confirm(
							`文件数量为 ${fileList.length} 个，是否确认上传？`,
							'提示',
							{
	...
	const uploadFile = (file) => {
		...
		uploadingFileMap.set(
			fileIndex,
			uploadingFileList[uploadingFileList.length - 1]
		)
		...
		uploadFileReq(param)
			.then((res) => {
				...
				if (
					proxyUploadType.includes(fileDataStore.currentStorageSource.type.key)
				) {
					fileProxyUpload(file, res.data, fileIndex)
				} else if (
					s3UploadType.includes(fileDataStore.currentStorageSource.type.key)
				) {
					s3FileUpload(file, res.data, fileIndex)
				} else if (
					onedriveUploadType.includes(
						fileDataStore.currentStorageSource.type.key
					)
				) {
				...
				.put(`${uploadUrl}`, fileBlock, {
					...
					onUploadProgress: (progressEvent) => {
						if (progressEvent.lengthComputable) {
							...
							uploadFileInfo.progress = Math.round(
								(realLoaded / fileSize) * 100
							)
							uploadFileInfo.speed = common.fileSizeFormat(
								Math.round(
									(realLoaded / (Date.now() - uploadFileInfo.startTime)) * 1000
								)
							)
						}
					},
	...
	const baseOnUploadProgress = (progressEvent, fileIndex, isProxyUpload) => {
		...
		uploadFileInfo.speed = common.fileSizeFormat(
			Math.round(
				(progressEvent.loaded / (Date.now() - uploadFileInfo.startTime)) * 1000
			)
		)
	...
	if (!isInitWatch) {
		watch(
			() => uploadProgressInfoStatistics.value.totalUploading,
			(newValue) => {
