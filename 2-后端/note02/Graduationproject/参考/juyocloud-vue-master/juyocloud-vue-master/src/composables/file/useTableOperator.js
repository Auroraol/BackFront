...
window.addEventListener(
	...
	function (e) {
		if (e.key === 'Escape' && allowShortcuts()) {
			...
			// 如果按了删除键，且当前状态允许快捷键操作
			if (batchDelete && selectRows?.value?.length > 0) {
				e.preventDefault()
				batchDelete()
			}
		} else if (e.key === 'Backspace' && allowShortcuts()) {
			// 如果按了删除键，且当前状态允许快捷键操作
			if (
				fileDataStore.fileList.length > 0 &&
				fileDataStore.fileList[0].type === 'BACK'
			) {
...
export const hasPasswordInputFocus = () => {
	return (
		document.querySelector('.is-message-box .el-input__inner') ===
...
export default function useTableOperator() {
	...
	tableClickRow = (row, event) => {
		...
		if (
			!isClickSelection &&
			storageConfigStore.globalConfig.fileClickMode === 'click'
		) {
			openRow(row)
			return
		}
		...
		let isClickSelf =
			selectRows.value.length === 1 && selectRow.value?.name === row.name

		// 如果按住了 shift 选中
		if (shiftState.value) {
			// 上一个选择的 index
			let prevSelectIndex = fileDataStore.fileList.findIndex(
				(value) => value.name === selectRow.value.name
			)

			// 如果选中行之前也按 shift 选中过行，表示要进行多选
			if (prevSelectIndex !== null) {
				let currentShiftIndex = fileDataStore.fileList.findIndex(
	...
	const tableHoverRow = (row, column, cell, event) => {
		...
		if (event.buttons === 1 && startDragClickIndex.value !== -1) {
			...
			let oldEndDragClickIndex =
				endDragClickIndex.value >= 0
					? endDragClickIndex.value
					: newEndDragClickIndex - 1
