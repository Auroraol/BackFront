...
export default function useFileOperator() {
    ...
    const batchDownloadFile = (row) => {
        ...
        if (row?.name) {
            confirmMsg = `确定下载 <span class="text-blue-500">${row.name}</span> ？`
        } else if (selectRows.value.length === 1) {
            confirmMsg = `确定下载 <span class="text-blue-500">${selectRows.value[0].name}</span> ？`
        ...
        ElMessageBox.confirm(confirmMsg, '下载文件', {
    ...
    const newFolder = (name) => {
        ...
        return newFolderReq(param).finally(() => {
            loadFile()
        })
    }
    ...
    const batchDelete = () => {
        ...
        let deleteConfirmMsg =
            selectRows.value.length === 1 ? '是否确认删除 ' : '是否确认批量删除 '

        let notSupportDeleteNotEmptyFolderType = [
            's3',
            'tencent',
            'aliyun',
            'qiniu',
        ...
        if (
            selectFolders.value.length > 0 &&
            notSupportDeleteNotEmptyFolderType.includes(
