import {
	loadStoragePasswordReq,
	saveStoragePasswordReq,
...
export default function useStoragePassword(router, route) {
	...
	const savePasswordData = () => {
		let notFill = passwordList.value.find((value) => {
			if (
				util.isEmpty(value.description) ||
				util.isEmpty(value.expression) ||
				util.isEmpty(value.password)
			) {
		...
		if (!notFill) {
			...
			saveStoragePasswordReq(currentStorageId, passwordList.value)
				.then(() => {
