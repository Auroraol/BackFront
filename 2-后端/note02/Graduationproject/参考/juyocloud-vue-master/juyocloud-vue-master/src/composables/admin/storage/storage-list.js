...
export default function useStorageList(router) {
	...
	const handleOperator = (command) => {
		switch (command.operator) {
			case 'edit':
				editStorage(command.storage)
				break
			case 'enable':
				switchEnableStatus(command.storage)
				break
			case 'readmeManager':
				showReadmePage(command.storage)
				break
			case 'cacheManager':
				cacheManage(command.storage)
				break
			case 'filterManager':
				showFilterPage(command.storage)
				break
			case 'pwdManager':
				showPasswordPage(command.storage)
				break
			case 'delete':
				deleteStorage(command.storage)
				break
		}
	}
	...
	const storageListSearchResult = computed(() => {
		...
		return storageList.value.filter((item) => {
			if (
				item.name &&
				item.name.toLowerCase().indexOf(searchKey.value.toLowerCase()) !== -1
			) {
				return true
			}
			if (
				item.remark &&
				item.remark.toLowerCase().indexOf(searchKey.value.toLowerCase()) !== -1
			) {
				return true
			}
			if (
				...
				item.type.description
					.toLowerCase()
					.indexOf(searchKey.value.toLowerCase()) !== -1
			) {
				return true
			}
			if (
				item.type &&
				item.type.key.toLowerCase().indexOf(searchKey.value.toLowerCase()) !==
					-1
			) {
				return true
			}
