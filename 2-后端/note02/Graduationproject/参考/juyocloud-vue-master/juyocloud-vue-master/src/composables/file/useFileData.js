...
if (skeletonData.length === 0) {
	// 动态配置骨架屏行数
	for (let i = 0; i < globalConfigStore.juyoConfig.skeleton.size; i++) {
		skeletonData.push({})
	}
}
// ------------- loading end   ------------
...
export default function useFileData() {
		...
		loadFileListReq(param)
			.then((response) => {
				...
				if (searchPath !== '' && searchPath !== '/') {
					let parentPathName = path.basename(
						path.resolve(currentPath.value, '../')
					)
					fileList.unshift({
				...
				if (fullpath.value) {
					title.value =
						storageConfigStore.globalConfig.siteName +
						' | ' +
						fullpath.value[fullpath.value.length - 1]
				} else {
	...
	const openRow = (row,page) => {
		...
		if (row.type === 'FILE') {
			...
			if (row.type === 'ROOT') {
				...
				let fullPath = removeDuplicateSlashes(
					'/' + storageKey.value + '/' + row.path
				)
				fullPath = encodeAllIgnoreSlashes(fullPath)
				routerRef.value.push(fullPath)
			} else {
				let fullPath = removeDuplicateSlashes(
					'/' + storageKey.value + '/' + row.path + '/' + row.name
				)
				fullPath = encodeAllIgnoreSlashes(fullPath)
				routerRef.value.push(fullPath)
			}
	...
	let skeletonLoading = computed(() => {
		let skeletonEnable = globalConfigStore.juyoConfig.skeleton.enable
		let skeletonShow = globalConfigStore.juyoConfig.skeleton.show
	...
	let popPassword = () => {
		...
		ElMessageBox.prompt('此文件夹已加密，请输入密码', '提示', {
			...
			.catch(() => {
				if (
					(searchParam.path === '/' || searchParam.path === '') &&
					storageConfigStore.globalConfig.rootShowStorage === true
				) {
