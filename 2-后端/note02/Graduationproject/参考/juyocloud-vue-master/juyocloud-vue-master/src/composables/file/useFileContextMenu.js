...
export default function useFileContextMenu(currentInstance) {
    const showFileMenu = (row, column, event) => {
        ...
        if (row instanceof Event) {
            event = row
            if (!event.target.classList.contains('page-main-wrap') &&
                !event.target.classList.contains('toolbar') &&
                !event.target.classList.contains('el-breadcrumb') &&
                !event.target.classList.contains('page-main') &&
                !event.target.classList.contains('jy-pan-body')) {
                return
            }
