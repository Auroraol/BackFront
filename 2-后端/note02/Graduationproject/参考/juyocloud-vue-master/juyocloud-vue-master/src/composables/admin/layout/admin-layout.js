...
export default function useAdminLayout(router, route) {
	...
	// 从 github 获取最新信息
	let { data: githubLatestInfo, loading: githubLatestLoading } = useRequest(
		githubInfoReq,
		{
			errorRetryCount: 3,
		}
	...
	const logout = () => {
		logoutReq().then(() => {
			...
			storageConfigStore().updateLoginStatus(false)
			router.push('/login')
		})
	}
	...
	const logDownload = () => {
		...
		downloadLogReq()
			.then((res) => {
				...
				const fileName = `JCLOUD诊断日志 - ${dayjs().format(
					'YYYY-MM-DD HH:mm:ss'
				)}.zip`
	...
	const rebuildTitle = () => {
		...
		// 获取当前选中的左侧导航的文字, 作为标题
		document.title = (route.meta.name || '橘柚云盘') + ' | 后台管理'
	}
