    ...
    >
      <template #form-title>
        <div class="flex justify-between">
          ...
          <el-tooltip
              placement="top"
              content="启用后下发的规则设置会失效，仅会为读取目录下的 readme.md 文件来渲染文档, 且固定在底部显示."
          >
            <div>
              <span class="mr-4 text-sm text-gray-500">兼容为读取 readme.md</span>
              <el-switch
                  v-model="compatibilityReadme"
                  active-text="是"
                  inline-prompt
                  inactive-text="否"
                  @change="changeCompatibilityReadmeStatus"
              ></el-switch>
            </div>
          </el-tooltip>
        </div>
      ...
      <template #form-sub-title>
        <el-alert :closable="false" type="info">
          ...
          <div class="rules-tips">
            <b>/</b>: 根目录, 如
            <span class="code select-all">/</span> 表示根路径下会加载此文档显示.
          </div>
          <div class="rules-tips">
            <b>*</b>：单级路径通配符，如表达式
            <span class="code select-all">/*</span>
            表示根路径下的直接子文件夹会加载此文档显示.
          </div>
          <div class="rules-tips">
            <b>**</b>：多级路径通配符，如表达式
            <span class="code select-all">/music/**</span> 表示 /music
            文件夹及以下所有文件夹都会加载此文档.
          </div>
          <div class="rules-tips">
            注：系统匹配到第一个符合的规则就会取文档进行显示，所以请调整好规则顺序，下方规则可进行拖拽排序。
          </div>
      ...
      >
        <div class="sm:flex sm:space-x-2 sm:border-b-0 sm:pb-0 border-b pb-2">
          ...
          ></el-input>
          <el-input
              v-model="item.readmeText"
              class="editor-input"
              readonly
              :prefix-icon="Document"
              placeholder="请点击编辑文档"
              @click="openReadmeEditor(item)"
          ></el-input>
          <el-select v-model="item.displayMode" class="editor-input">
...
import {
	changeCompatibilityReadmeEnableReq,
	loadStorageItemReq,
