<template>
	<div
		ref="zFormItem"
		:class="parentForm.formError[props.prop] ? 'is-error' : 'is-success'"
		class="z-form-item mt-6 sm:mt-5 space-y-6 sm:space-y-5"
	>
		<div
			class="sm:grid sm:grid-cols-3 sm:gap-4 sm:items-start sm:border-b sm:border-gray-200 sm:pb-5"
		>
			<label
				v-if="label"
				:for="props.prop"
			...
			<div class="mt-1 sm:mt-0 sm:col-span-2">
				<div
					class="input max-w-2xl flex rounded-md z-form-item-input"
					@input="changeInput"
				>
					<slot></slot>
				</div>
				<p
					class="empty:mt-0 mt-2 text-sm max-w-2xl text-gray-400 z-form-item-tips"
				>
					<slot name="tips"></slot>
				</p>
