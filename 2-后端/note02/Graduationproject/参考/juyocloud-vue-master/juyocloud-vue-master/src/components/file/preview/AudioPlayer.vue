...
let initAudio = (data, index) => {
	...
	if (ap) {
		...
		el.getElementsByClassName('aplayer-icon-order')[0].setAttribute(
			'title',
			'播放顺序'
		)
		el.getElementsByClassName('aplayer-icon-loop')[0].setAttribute(
			'title',
			'循环模式'
		)
		el.getElementsByClassName('aplayer-icon-menu')[0].setAttribute(
			'title',
			'播放列表'
		)
