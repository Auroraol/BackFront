<template>
  <form class="contaier">
    ...
    <div class="uploadimg">
      ...
      <div>
        <el-upload
          class="upload-demo"
          :drag="true"
          :action="pinia.apiRoot + '/api/uploadarticle'"
          :limit="1"
          :auto-upload="false"
          list-type="picture"
          ref="uploadCover"
    ...
    <div class="article">
      <md-editor
        @input="saveEditor"
        v-model="articleWrite.articleText"
        :toolbarsExclude="['quote', 'save', 'htmlPreview']"
...
const uploadArticle = (articleWrite: ArticleWrite) => {
  if (
    articleWrite.articleTags.length === 0 ||
    articleWrite.articleTitle === "" ||
    articleWrite.articleText === ""
  ) {
    ElMessage({
      showClose: true,
  ...
  if (coverNum == 0) {
    ...
    formArticle.append(
      "writeArticle",
      new Blob([JSON.stringify(articleWrite)], { type: "application/json" })
    );
    useAxios
      .post("/uploadarticle", formArticle)
      .then((res) => {
  ...
  .uploadArticle {
    ...
    div {
      ...
      &:hover {
        ...
        box-shadow: 0.3rem 0.3rem var(--box-shadow);
      }
    }
  }
