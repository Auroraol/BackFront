...
Elasticsearch 是一个实时的分布式存储、搜索、分析的引擎。可以用来实现搜索、日志统计、分析、系统监控等功能
...
搜索：百度，网站的站内搜索(新闻网站内搜索，电商商品搜索)，IT系统的检索数据分析：比如最近一个月销量前10的商品，或者说最近一周的交易额

**全文检索 结构化检索**

全文检索：比如我想查询标题“包含新冠病毒”的文章（类似于 where title like '%包含新冠病毒%')结构化检索：比如我我想查询水和乳的销量。（类似于按照类型查询 wheretype in ("水","乳"))部分匹配、自动完成、搜索纠错、搜索推荐

**海量数据近乎实时的处理**

分布式：ES内部可以将海量数据分散到多台服务器上去存储和检索。海量数据的处理:分布式以后,就可以采用大量的服务器去存储和检索数据,自然而然就可以实现海量数据的处理了近实时:秒级别对数据进行搜索和分析
...
- 在GitHub搜索代码
  ...
  ![image-20210720193623245](elasticearch.assets/image-20210720193623245.png)

- 在百度搜索答案

  ![image-20210720193641907](elasticearch.assets/image-20210720193641907.png)
...
+ 学习曲线陡峭
+ 不支持水平扩展
+ lucene他是单机应用,只能在单台服务器上使用,最多只能处理单台服务器可以处理的数据量

##  Near Realtime (NRT)近实时

<font color = red>从写入数据到数据可以被搜索到有一个小延迟（大概1秒）</font>但是基于es执行搜索和分析可以达到秒级

![ES 写入到查询的1秒钟延迟原理图.png](elasticearch.assets/1592968864181-b41318cb-47cf-4c92-a026-069fc846562c.png)

## elastic stack（ELK）

elasticsearch是elastic stack的核心，负责存储、搜索、分析数据。包括beats、Logstash、kibana、elasticsearch

<img src="elasticearch.assets/image-20231025214911124.png" alt="image-20231025214911124" style="zoom:67%;" />
...
<img src="elasticearch.assets/image-20231025213908562.png" alt="image-20231025213908562" style="zoom:150%;" />

**倒排索引的概念是基于MySQL这样的正向索引而言的。**

### 正向索引(mysql采用)

那么什么是正向索引呢？例如给下表（tb_goods）中的id创建索引：![image-20210720195531539](elasticearch.assets/image-20210720195531539.png)
...
![image-20210720200457207](elasticearch.assets/image-20210720200457207.png)
...
![image-20210720201115192](elasticearch.assets/image-20210720201115192.png)
...
<img src="elasticearch.assets/image-20231025215802857.png" alt="image-20231025215802857" style="zoom: 50%;" />

一个document里面有多个field((Json文档中的字段))，每个field就是一个数据字段, 类似于数据库中的列
...
<img src="elasticearch.assets/image-20210720203022172.png" alt="image-20210720203022172" style="zoom: 50%;" />
...
primary shard （建立索引时一次设置，不能修改，默认5个）， replica shard（随时修改数量，默认1个），默认
每个索引10个shard, 5个primary shard, 5个replica shard，最小的高可用配置，是2台服务器。
结合一张图，表示这几个元素直接的关系。

![es shard  replica  以及横向扩展.png](elasticearch.assets/1591083002766-6603a0a3-e35d-4a41-9059-7e84d67d27fd.png)

# 安装es、kibana

具体见: [安装](C:\Users\16658\Documents\GitHub\java_note\note03\Elasticearch\安装)

链接：https://pan.baidu.com/s/11LwU0sOZZqDQLQFXI0TNiQ 
提取码：12nq
...
首先要清楚es是面向文档的（也就是面向json串的）。它和之前的面向数据库编程不太一样。
例子:  
...
常见的做法是拆解两个表: employee表， employee_info表，将employee对象的数据重新拆开来，变成Employee数据和Employeelnfo数据
...
所以ES是面向文档的，文档中存储的json数据结构，与面向对象的数据结构是一样的，基于这种文档数据结构，es可以提供复杂的索引，全文检索，分析聚合等功能

## mapping映射属性

![img](elasticearch.assets/1645690693275-f39522a1-2c0a-4bfe-9822-80d55adcfc2d.png)

Mapping类似数据库中的schema的定义，作用如下

+ 定义索引汇总字段的名称
+ 定义字段的数据类型，如字符串、数字，时间
...
![image-20210721171655308](elasticearch.assets/image-20210721171655308.png)
...
![DKV9HZbVS6](elasticearch.assets/DKV9HZbVS6.gif)
...
![vZrdKAh19C](elasticearch.assets/vZrdKAh19C.gif)
...
![image-20210721175443234](elasticearch.assets/image-20210721175443234.png)
...
![image-20210720220400297](elasticearch.assets/image-20210720220400297.png) 
...
![image-20210720223049408](elasticearch.assets/image-20210720223049408.png)
...
![image-20210720230811674](elasticearch.assets/image-20210720230811674.png)
...
![image-20210720231040875](elasticearch.assets/image-20210720231040875.png)
...
![image-20210721221744883](elasticearch.assets/image-20210721221744883.png)
...
![image-20210721222057212](elasticearch.assets/image-20210721222057212.png)
...
![image-20210721223859419](elasticearch.assets/image-20210721223859419.png)

点击搜索按钮，可以看到浏览器控制台发出了请求：

![image-20210721224033789](elasticearch.assets/image-20210721224033789.png)
...
![image-20210722091940726](elasticearch.assets/image-20210722091940726.png)

传递的参数如图：

![image-20210722092051994](elasticearch.assets/image-20210722092051994.png) 
...
![image-20210722092935453](elasticearch.assets/image-20210722092935453.png)
...
![image-20210722093414542](elasticearch.assets/image-20210722093414542.png) 

并且，在前端会发起查询请求，将你的坐标发送到服务端：

![image-20210722093642382](elasticearch.assets/image-20210722093642382.png) 
...
![image-20210722095227059](elasticearch.assets/image-20210722095227059.png)
...
![image-20210722095902314](elasticearch.assets/image-20210722095902314.png)
...
![image-20210722100040674](elasticearch.assets/image-20210722100040674.png)
...
![image-20210722100613966](elasticearch.assets/image-20210722100613966.png)



重启后测试，发现页面能成功显示距离了：

![image-20210722100838604](elasticearch.assets/image-20210722100838604.png)
...
![image-20210722100947292](elasticearch.assets/image-20210722100947292.png)
...
![image-20210722101908062](elasticearch.assets/image-20210722101908062.png)
...
![image-20210721191544750](elasticearch.assets/image-20210721191544750-16982485203072.png)



对应的JavaAPI如下：

![image-20210722102850818](elasticearch.assets/image-20210722102850818.png)
